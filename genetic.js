...
crossOver : function(parentA, parentB) {
	var cutPointA = this.random(0,parentA.neurons.length-1);
	var cutPointB = this.random(0,parentB.neurons.length-1);
	while (cutPointA==0 || cutPointA==cutPointB || cutPointA==parentA.neurons.length-1 || cutPointB==0 || cutPointB==parentB.neurons.length-1){
		cutPointA = this.random(0,parentA.neurons.length-1);
		cutPointB = this.random(0,parentB.neurons.length-1);
	}
	
	for (var i = cutPointA; i < parentA.neurons.length; i++){
	for(var j = cutPointB; j<parentB.neurons.length; j++){
	var biasFromParentA = parentA.neurons[i]['bias'];
	var biasFromParentB = parentA.neurons[j]['bias'];
	
parentA.neurons[i]['bias'] = parentB.neurons[i]['bias'];
parentB.neurons[i]['bias'] = biasFromParentA;
parentA.neurons[j]['bias'] = parentB.neurons[j]['bias'];
parentB.neurons[j]['bias'] = biasFromParentB;
}
}
